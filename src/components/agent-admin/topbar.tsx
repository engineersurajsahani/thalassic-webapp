...
export default function AgentAdminTopbar() {
  ...
  const isHome =
    pathname === "/agent-admin/dashboard" || pathname === "/partner-admin";

  return (
    <header
      className={`h-16 shrink-0 border-b flex items-center justify-between px-6 gap-4 z-20 sticky top-0 transition-colors duration-200 ${isDark ? "bg-[#0B0F19] border-[#1F2937]" : "bg-[#FFFFFF] border-[#E5E7EB]"}`}
    >
      {/* Left */}
      <div className="flex flex-col justify-center min-w-0">
        {isHome ? (
          <>
            ...
            >
              {getGreeting()}, {user?.name || "Partner Admin"}
            </p>
          </>
        ) : (
          <div
      ...
      <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className={`relative w-8 h-8 flex items-center justify-center rounded-lg transition-colors duration-200 cursor-pointer ${
              isDark
                ...
                : "text-[#6B7280] hover:bg-[#EEF1FE] hover:text-[#3D5EF6]"
            }`}
        ...
        <div
          className={`w-px h-5 mx-1 ${isDark ? "bg-[#374151]" : "bg-[#E5E7EB]"}`}
        />
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="hidden sm:flex flex-col items-start leading-tight">
            ...
            <span
              className={`text-[10px] ${isDark ? "text-gray-400" : "text-[#6B7280]"}`}
            >
              Manning Operations
            </span>
          </div>
