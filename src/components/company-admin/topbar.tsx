...
const pageNames: Record<string, string> = {
  ...
  "/company-admin/seafearers": "Seafarers",
  "/company-admin/courses":    "Courses",
  "/company-admin/documents":  "Documents",
  "/company-admin/support":    "Support",
};
...
export default function CompanyAdminTopbar() {
  ...
  return (
    <header
      className={`h-16 shrink-0 border-b flex items-center justify-between px-6 gap-4 ${
        isDark ? "bg-[#0d1f35] border-white/5" : "bg-white border-slate-200"
      }`}
    >
      {/* Left */}
      <div className="flex flex-col justify-center min-w-0">
        {isHome ? (
          <>
            ...
            <p className={`text-sm font-semibold leading-tight ${isDark ? "text-white/75" : "text-slate-700"}`}>
              {getGreeting()}, Admin
            </p>
          </>
        ) : (
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        <button
          ...
          className={`w-8 h-8 flex items-center justify-center rounded-lg transition-colors ${
            isDark ? "text-yellow-300 hover:bg-white/8" : "text-slate-500 hover:bg-slate-100"
          }`}
        >
