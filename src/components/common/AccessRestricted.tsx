  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Notice Description */}
        <p
          className={`text-xs leading-relaxed mb-6 ${isDark ? "text-white/70" : "text-[#6B7280]"}`}
        >
          In accordance with{" "}
          <strong>PRD Section 4.13 (Partner Access Restrictions)</strong>,
          Partner Admin users are restricted from accessing system-level
          administrative features, master reports, audit trails, commission
          calculations, or information belonging to other Partner organizations.
        </p>

        {/* Restricted List Box */}
        <div
          className={`p-4 rounded-[12px] mb-6 border ${isDark ? "bg-[#FEE2E2]/10 border-red-500/20" : "bg-[#FEE2E2]/30 border-red-100"}`}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
            {restrictedItems.map((item, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <span className={isDark ? "text-white/80" : "text-[#111827]"}>
                  {item}
                </span>
              </div>
            ))}
        ...
        {/* Permitted Scope */}
        <div
          className={`p-4 rounded-[12px] mb-6 border ${isDark ? "bg-[#EEF1FE]/10 border-[#3D5EF6]/20" : "bg-[#EEF1FE]/40 border-blue-100"}`}
        >
          <h3 className="text-xs font-bold text-[#3D5EF6] uppercase tracking-wider mb-2 flex items-center gap-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-[#3D5EF6]" /> Permitted
            Role Access
          </h3>
          <p
            className={`text-xs ${isDark ? "text-white/70" : "text-[#6B7280]"}`}
          >
            Partner Admin users shall only access information belonging to their
            own Partner organization:{" "}
            <strong>
              Dashboard, Partners, Invoices, Settlements, Reports, and Profile
            </strong>
            .
          </p>
        </div>
