...
export default function CourseCatalogPage() {
  ...
  return (
    <div className={`min-h-screen font-outfit transition-colors duration-500 ${
      ...
      {/* ================= HERO ================= */}
      <section className={`relative pt-[120px] pb-8 border-b ${
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <ScrollReveal>
            ...
            {/* Heading */}
            <h1 className={`text-4xl md:text-5xl font-black mb-3 bg-clip-text text-transparent bg-gradient-to-r ${
              isDark ? "from-white via-slate-100 to-slate-350" : "from-slate-900 via-blue-950 to-slate-800"
            }`}>
              Professional Maritime Courses
            </h1>

            <p className={`max-w-3xl text-sm leading-relaxed font-light ${
            ...
            <div className="mt-8 relative max-w-3xl">
              ...
              <input
                ...
                className={`w-full pl-14 pr-5 py-4 rounded-2xl border text-sm transition-all shadow-lg font-medium focus:outline-none focus:ring-1 focus:ring-blue-500/20 ${
                  isDark
            ...
            <div className="flex gap-10 mt-10 pt-6 border-t border-slate-800/10 dark:border-slate-800/50">
              {[
                ...
                <div key={i}>
                  <div className={`text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r ${
                    isDark ? "from-blue-400 to-slate-400" : "from-blue-600 to-indigo-650"
                  }`}>
                    {stat.value}
                  </div>
          ...
          <aside className="hidden lg:block space-y-7 h-fit sticky top-28">
            <ScrollReveal>
              <div className={`p-6 border rounded-3xl space-y-6 ${
                isDark ? "bg-[#0a1122]/70 border-slate-800" : "bg-white border-slate-200 shadow-sm"
              }`}>
                  ...
                  <div className="space-y-2.5">
                    {Object.entries(categoryLabels).map(([key, label]) => (
                      <label key={key} className="flex items-center gap-2.5 text-xs font-semibold cursor-pointer select-none group text-slate-400 hover:text-slate-200">
                        ...
                        />
                        <span className={`transition-colors text-xs font-medium ${
                          selectedCategories.includes(key) ? "text-blue-400 font-black" : ""
                        }`}>
                          {label}
                        </span>
                      </label>
                  ...
                  <div className="space-y-2.5">
                    {uniqueLevels.map((level) => (
                      <label key={level} className="flex items-center gap-2.5 text-xs font-semibold cursor-pointer select-none group text-slate-400 hover:text-slate-200">
                        ...
                        />
                        <span className={`transition-colors text-xs font-medium ${
                          selectedLevels.includes(level) ? "text-blue-400 font-black" : ""
                        }`}>
                          {level}
                        </span>
                      </label>
              ...
              <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border rounded-2xl gap-4 ${
                ...
                <div className="flex items-center gap-3.5 w-full sm:w-auto justify-between sm:justify-end">
                  ...
                  <button
                    ...
                    className={`lg:hidden flex items-center gap-1.5 px-3 py-1.5 border rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer ${
                      isDark ? "bg-white/5 border-slate-800 hover:bg-white/10" : "bg-slate-55 border-slate-200 hover:bg-slate-100"
                    }`}
                  >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className={`px-3 py-1.5 text-xs font-black uppercase tracking-wider rounded-xl border focus:outline-none cursor-pointer ${
                        isDark ? "bg-[#050a14] border-slate-850 text-slate-300" : "bg-slate-50 border-slate-200 text-slate-850"
                      }`}
                    >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {filteredCourses.map((course, idx) => (
                <ScrollReveal key={course.id} delay={(idx % 2) * 150}>
                  <div
                    className={`group border rounded-3xl overflow-hidden transition-all duration-500 flex flex-col justify-between h-full hover:-translate-y-1.5 ${
                      isDark 
                  ...
                  >
                    <div className="relative w-full h-44 z-0">
                      ...
                      <span className={`absolute top-4 left-4 px-2.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shadow-sm ${
                        course.category === "basic" ? "bg-blue-650 text-white" :
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`text-[9px] px-2 py-0.5 border rounded-lg font-black uppercase tracking-wider ${
                            isDark ? "bg-white/5 border-slate-800 text-slate-400" : "bg-slate-50 border-slate-200 text-slate-650"
                          }`}>
                            {course.level}
                          </span>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex lg:hidden bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className={`w-80 max-w-[85vw] h-full p-6 overflow-y-auto space-y-6 flex flex-col justify-between ${
                ...
                <div className="space-y-2.5">
                  {Object.entries(categoryLabels).map(([key, label]) => (
                    <label key={key} className="flex items-center gap-2.5 text-xs font-semibold cursor-pointer select-none group text-slate-400">
                      ...
                      />
                      <span className={`transition-colors text-xs font-medium ${
                        selectedCategories.includes(key) ? "text-blue-450 font-black" : ""
                      }`}>
                        {label}
                      </span>
                    </label>
                ...
                <div className="space-y-2.5">
                  {uniqueLevels.map((level) => (
                    <label key={level} className="flex items-center gap-2.5 text-xs font-semibold cursor-pointer select-none group text-slate-400">
                      ...
                      />
                      <span className={`transition-colors text-xs font-medium ${
                        selectedLevels.includes(level) ? "text-blue-405 font-black" : ""
                      }`}>
                        {level}
                      </span>
                    </label>
            ...
            <div className="pt-6 border-t border-slate-850 flex gap-4">
              <button
                onClick={handleClearFilters}
                className={`w-full py-2.5 text-[10px] font-black uppercase tracking-wider rounded-xl border ${
                  isDark ? "bg-white/5 border-slate-800 hover:bg-white/10" : "bg-slate-50 border-slate-200 hover:bg-slate-100"
                }`}
              >
                Reset
              </button>
