...
export default function AboutUsPage() {
  ...
  return (
    <div className={`transition-colors duration-500 min-h-screen font-outfit ${isDark ? "bg-[#050a14] text-slate-100" : "bg-[#f8fafc] text-slate-900"
      }`}>

      {/* Hero Header Section */}
      <section className="relative max-w-7xl mx-auto px-6 pt-[88px] md:pt-[96px] pb-16 text-center md:text-left">
        <ScrollReveal>
          <span className={`inline-block px-4 py-1.5 rounded-full text-xs font-black tracking-widest uppercase mb-4 border ${isDark
              ? "bg-blue-950/40 text-blue-300 border-blue-500/10"
              : "bg-blue-50 text-blue-600 border-blue-200"
            }`}>
            // About Our Platform
          </span>
          <h1 className={`text-4xl md:text-6xl font-black tracking-tight mb-6 bg-clip-text text-transparent bg-gradient-to-r ${isDark ? "from-white via-slate-100 to-slate-350" : "from-slate-900 via-blue-950 to-slate-800"
            }`}>
            Hari Om Thalassic
          </h1>
          <p className={`text-base md:text-lg max-w-3xl leading-relaxed font-light ${isDark ? "text-slate-300" : "text-slate-600"
            }`}>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-8">
        ...
        <ScrollReveal delay={100}>
          <div className={`border rounded-3xl p-8 shadow-xl transition-all duration-500 hover:scale-[1.01] ${isDark
              ? "bg-[#0a1122]/70 border-slate-800 hover:border-blue-500/20"
              : "bg-white border-slate-200 shadow-slate-200/50"
            }`}>
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 border ${isDark
      ...
      <section className={`py-20 border-y ${isDark
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        <ScrollReveal>
          <div className={`rounded-3xl p-8 border flex flex-col md:flex-row justify-between items-start md:items-center gap-6 ${isDark
            ...
            <div className="flex gap-4 w-full md:w-auto">
              <Link href="/courses" className={`w-full md:w-auto px-6 py-3.5 text-center text-xs font-black rounded-2xl transition-all shadow-md ${isDark ? "bg-gradient-to-r from-blue-600 to-slate-600 text-white hover:from-blue-500" : "bg-white hover:bg-slate-50 text-blue-600"
                }`}>
                Browse Courses
              </Link>
              <Link href="/contact" className={`w-full md:w-auto px-6 py-3.5 text-center text-xs font-black rounded-2xl transition-all border ${isDark
