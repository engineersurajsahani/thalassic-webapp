...
export default function PartnerAdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [pricingData, dashData] = await Promise.all([
        ...
        agentAdminService.getDashboardData().catch(() => null),
      ]);
  ...
  const pendingApprovals = pricings.filter(
    ...
    <div className="space-y-8 animate-fadeIn pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1
              className={`text-2xl md:text-3xl font-extrabold tracking-tight ${headingText}`}
            >
              Partner Admin Management
            </h1>
          </div>
          <p className={`text-xs md:text-sm mt-1 ${subText}`}>
            Supervise partner operations, approve custom course pricing
            proposals, and verify remittances.
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`p-5 ${cardBg}`}>
          ...
          <p className={`text-2xl font-black mt-3 ${headingText}`}>
            ₹
            {Number(
              dashboardStats?.stats?.amountSettled || 185000,
            ).toLocaleString("en-IN")}
          </p>
          <p className={`text-[10px] mt-1 ${subText}`}>
            Verified partner transfers
          </p>
        </div>
        ...
        <div className={`p-5 ${cardBg}`}>
          ...
          <p className={`text-2xl font-black mt-3 ${headingText}`}>
            ₹
            {Number(
              dashboardStats?.stats?.outstandingAmount || 45000,
            ).toLocaleString("en-IN")}
          </p>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <h3
            className={`text-base font-extrabold group-hover:text-[#3D5EF6] transition-colors ${headingText}`}
          >
            Partner Pricing Approvals
          </h3>
          <p className={`text-xs mt-1.5 leading-relaxed ${subText}`}>
            Review pending price changes, compare standard vs. proposed payable
            amounts, and approve/reject with audit logs.
          </p>
        ...
        >
          ...
          <h3
            className={`text-base font-extrabold group-hover:text-emerald-600 transition-colors ${headingText}`}
          >
            Settlement Verification
          </h3>
          <p className={`text-xs mt-1.5 leading-relaxed ${subText}`}>
            Inspect bank remittance receipts, verify UTR transaction IDs, and
        ...
        >
          ...
          <h3
            className={`text-base font-extrabold group-hover:text-purple-600 transition-colors ${headingText}`}
          >
            Partner Operations Portal
          </h3>
          <p className={`text-xs mt-1.5 leading-relaxed ${subText}`}>
            Access the live partner candidate search, purchase entry tool, and
            financial reconciliation.
          </p>
          <div className="mt-4 flex items-center gap-1 text-xs font-bold text-purple-600">
            Open Operations <ArrowRight className="w-4 h-4" />
          </div>
        </Link>
      </section>
      ...
      <section className={`p-6 ${cardBg}`}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className={`text-base font-extrabold ${headingText}`}>
              Pending Pricing Proposals
            </h2>
            <p className={`text-xs ${subText}`}>
              Proposals awaiting Master/Admin approval
            </p>
          </div>
          <Link
            href="/agent-admin/pricing"
        ...
        {pendingApprovals.length === 0 ? (
          <div
            className={`p-8 text-center border border-dashed rounded-2xl ${isDark ? "border-white/10 text-gray-400" : "border-gray-200 text-gray-500"}`}
          >
            <CheckCircle2 className="w-8 h-8 mx-auto text-emerald-500 opacity-80 mb-2" />
            <p className="text-sm font-semibold">
              No pending pricing approval requests!
            </p>
            <p className="text-xs mt-1 text-gray-400">
              All partner course pricing rules are up to date.
            </p>
          </div>
        ) : (
              ...
              >
                <div>
                  ...
                  <p className={`text-[11px] mt-1 ${subText}`}>
                    Category: {item.category} | Standard Fee: ₹
