...
import {
  ...
  Filter, Calendar, Download, IndianRupee, FileText, Eye, X,
} from "lucide-react";

// ─── Mock Data ────────────────────────────────────────────────────────────────

const INVOICES = [
  { id: "INV-2026-001", seafarer: "Raj Kumar",       course: "STCW Basic Safety",     amount: 5000,  status: "Paid",    issued: "Jul 20, 2026", due: "Jul 27, 2026" },
  { id: "INV-2026-002", seafarer: "Priya Sharma",    course: "Deck Watchkeeping",      amount: 5800,  status: "Unpaid",  issued: "Jul 19, 2026", due: "Jul 26, 2026" },
  { id: "INV-2026-003", seafarer: "Amit Patel",      course: "Engine Room Watch",      amount: 8100,  status: "Paid",    issued: "Jul 18, 2026", due: "Jul 25, 2026" },
  { id: "INV-2026-004", seafarer: "Sunita Rajan",    course: "Advanced Fire Fighting", amount: 7200,  status: "Paid",    issued: "Jul 17, 2026", due: "Jul 24, 2026" },
  { id: "INV-2026-005", seafarer: "Karan Mehta",     course: "Ship Navigation",        amount: 6800,  status: "Overdue", issued: "Jul 15, 2026", due: "Jul 22, 2026" },
  { id: "INV-2026-006", seafarer: "Deepa Nair",      course: "Maritime Catering",      amount: 3500,  status: "Paid",    issued: "Jul 14, 2026", due: "Jul 21, 2026" },
  { id: "INV-2026-007", seafarer: "Vikram Das",      course: "Tanker Cargo Ops",       amount: 9400,  status: "Unpaid",  issued: "Jul 13, 2026", due: "Jul 20, 2026" },
  { id: "INV-2026-008", seafarer: "Meena Iyer",      course: "STCW Basic Safety",      amount: 5000,  status: "Paid",    issued: "Jul 12, 2026", due: "Jul 19, 2026" },
  { id: "INV-2026-009", seafarer: "Rohit Sharma",    course: "Tanker Cargo Ops",       amount: 9400,  status: "Paid",    issued: "Jul 11, 2026", due: "Jul 18, 2026" },
  { id: "INV-2026-010", seafarer: "Lakshmi Devi",    course: "Maritime Law",           amount: 3200,  status: "Overdue", issued: "Jul 10, 2026", due: "Jul 17, 2026" },
  { id: "INV-2026-011", seafarer: "Suresh Verma",    course: "Engine Room Watch",      amount: 8100,  status: "Paid",    issued: "Jul 9, 2026",  due: "Jul 16, 2026" },
  { id: "INV-2026-012", seafarer: "Anita Pillai",    course: "Advanced Navigation",    amount: 11000, status: "Paid",    issued: "Jul 8, 2026",  due: "Jul 15, 2026" },
  { id: "INV-2026-013", seafarer: "Mohammed Rafiq",  course: "STCW Basic Safety",      amount: 5000,  status: "Paid",    issued: "Jul 21, 2026", due: "Jul 28, 2026" },
  { id: "INV-2026-014", seafarer: "Tara Singh",      course: "Ship Navigation",        amount: 6800,  status: "Unpaid",  issued: "Jul 20, 2026", due: "Jul 27, 2026" },
];
...
export default function InvoicesPage() {
  ...
  const unpaidValue  = INVOICES.filter(i => i.status !== "Paid").reduce((s, i) => s + i.amount, 0);

  const kpis = [
    { label: "Total Invoices",  value: String(INVOICES.length),                             icon: Receipt,     bg: dk ? "bg-indigo-500/15"  : "bg-indigo-50",  color: "#6366f1" },
    { label: "Total Value",     value: "₹" + (totalValue / 1000).toFixed(0) + "K",          icon: IndianRupee, bg: dk ? "bg-sky-500/15"     : "bg-sky-50",     color: "#0ea5e9" },
    { label: "Collected",       value: "₹" + (paidValue / 1000).toFixed(0) + "K",           icon: CheckCircle2,bg: dk ? "bg-emerald-500/15" : "bg-emerald-50", color: "#10b981" },
    { label: "Outstanding",     value: "₹" + (unpaidValue / 1000).toFixed(0) + "K",         icon: AlertCircle, bg: dk ? "bg-rose-500/15"    : "bg-rose-50",    color: "#f43f5e" },
  ];

  const filtered = useMemo(() =>
    INVOICES.filter(inv => {
      const q = inv.seafarer.toLowerCase() + inv.id.toLowerCase() + inv.course.toLowerCase();
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className={`text-sm mt-0.5 ${mt}`}>Manage and track all course fee invoices</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={dk ? "border-b border-white/5" : "border-b border-slate-100"}>
                {["Invoice #","Seafarer","Course","Amount","Status","Issued","Due",""].map(h => (
            ...
            <tbody className={`divide-y ${divider}`}>
              {filtered.map((inv, i) => {
                ...
                return (
                  <tr key={inv.id} className={`transition-colors ${dk ? "hover:bg-white/[0.03]" : "hover:bg-slate-50"}`}>
                    <td className={`px-5 py-3.5 text-[12px] font-mono font-semibold ${dk ? "text-indigo-400" : "text-indigo-600"}`}>{inv.id}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <p className={`text-[13px] font-medium ${ht}`}>{inv.seafarer}</p>
                      </div>
                    </td>
                    ...
                    <td className={`px-5 py-3.5 text-[12px] ${mt}`}>
                      <div className="flex items-center gap-1.5"><Calendar className="w-3 h-3 shrink-0" />{inv.issued}</div>
                    </td>
                    <td className={`px-5 py-3.5 text-[12px] ${inv.status === "Overdue" ? (dk ? "text-red-400" : "text-red-600") : mt}`}>{inv.due}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button className={`p-1.5 rounded-lg transition-colors ${dk ? "hover:bg-white/8 text-white/30 hover:text-white/60" : "hover:bg-slate-100 text-slate-300 hover:text-slate-600"}`}>
          ...
          >
            ...
            <div className="flex items-start justify-between pb-4 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-[10px] font-semibold uppercase tracking-wider opacity-60">Tax Invoice</span>
                  <h3 className="text-base font-bold font-mono">{selectedInvoice.id}</h3>
                </div>
              </div>
              <button
                onClick={() => setSelectedInvoice(null)}
            ...
            <div className="py-4 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <div>
                  <span className="text-[10px] uppercase font-semibold opacity-40 block">Issued Date</span>
                  <span>{selectedInvoice.issued}</span>
                </div>
                <div>
                  <span className="text-[10px] uppercase font-semibold opacity-40 block">Due Date</span>
                  <span className={selectedInvoice.status === "Overdue" ? (dk ? "text-red-400" : "text-red-600") : ""}>{selectedInvoice.due}</span>
                </div>
              </div>

              <div className="p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-between">
                <div>
                  <span className="text-[10px] uppercase font-bold text-indigo-400 block">Invoice Amount</span>
                  <span className="text-base font-black text-indigo-400">₹{selectedInvoice.amount.toLocaleString("en-IN")}</span>
                </div>
