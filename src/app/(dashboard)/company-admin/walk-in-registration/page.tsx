...
const statusMeta: Record<
  string,
  { cls: [string, string]; icon: React.ReactNode }
> = {
  Active: {
    cls: ["text-emerald-700", "text-emerald-400"],
    icon: <CheckCircle2 className="w-3.5 h-3.5" />,
  },
  Pending: {
    cls: ["text-amber-700", "text-amber-400"],
    icon: <Clock className="w-3.5 h-3.5" />,
  },
  Dropped: {
    cls: ["text-red-700", "text-red-400"],
...
export default function WalkInRegistrationPage() {
  ...
  const [existingSeafarerId, setExistingSeafarerId] = useState<string | null>(
    null,
  );
  ...
  const stats = [
    ...
    {
      label: "Today",
      value: records.filter((r) => r.date === "Aug 2, 2026").length,
  ...
  const handleSearchExisting = () => {
    if (!form.email && !form.phone) {
      setValidationError(
        "Please enter an Email or Phone number to check for an existing seafarer.",
      );
      return;
    }
    ...
    // Simulate API delay
    setTimeout(() => {
      const found = mockSeafarers.find(
        (s) =>
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const newRecord = {
      id: existingSeafarerId
        ? existingSeafarerId
        : `WI-${String(records.length + 1).padStart(3, "0")}`,
      name: form.name,
      rank: form.rank,
      phone: form.phone,
      email: form.email,
      location: form.location,
      status: existingSeafarerId ? "Active" : "Pending",
      date: "Aug 2, 2026",
      cdcNo: form.cdcNo,
    };
        ...
        <div className={`${card} xl:col-span-2`}>
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center ${dk ? "bg-emerald-500/15" : "bg-emerald-50"}`}
              >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    value={form.cdcNo}
                    onChange={(e) =>
                      setForm((p) => ({ ...p, cdcNo: e.target.value }))
                    }
                ...
                {records.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`px-5 py-3.5 text-[12px] ${dk ? "text-white/55" : "text-slate-500"}`}
                      >
                        {r.rank}
                      </td>
                      <td className="px-5 py-3.5">
                        <span
                          className={`inline-flex items-center gap-1 text-[11px] font-semibold ${dk ? meta.cls[1] : meta.cls[0]}`}
                        >
                          {meta.icon}
                          {r.status}
                        </span>
                      ...
                      <td className="px-5 py-3.5">
                        <div
                          className={`flex items-center gap-1.5 text-[12px] ${mt}`}
                        >
