          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className={`inline-flex items-center gap-1 text-[11px] font-semibold ${statusCls[agent.status]}`}>
                  ...
                  {seafarers.map((sf, i) => (
                    <div key={sf.id} className={`rounded-xl p-3.5 ${cardBg}`}>
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className={`inline-flex items-center gap-1 text-[11px] font-semibold shrink-0 ${sfStatusCls[sf.status]}`}>
                          {sf.status}
                        </span>
                      </div>
                    ...
                    <div key={sf.id} className={`rounded-xl p-3.5 ${cardBg}`}>
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <span className={`inline-flex items-center gap-1 text-[11px] font-semibold mt-0.5 ${sf.commissionPaid ? (dk ? "text-emerald-400" : "text-emerald-700") : (dk ? "text-amber-400" : "text-amber-700")}`}>
...
export default function AgentsPage() {
  ...
  const filterActive = "text-indigo-600 dark:text-indigo-400 font-bold underline underline-offset-4 decoration-2 decoration-indigo-500";
  const filterInactive = dk ? "text-white/40 hover:text-white/80 font-medium" : "text-slate-500 hover:text-slate-800 font-medium";
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`inline-flex items-center gap-1 text-[11px] font-semibold shrink-0 ${statusCls[agent.status]}`}>
                      {agent.status}
                    </span>
                  </div>
          ...
          {requests.length === 0 ? (
            ...
            requests.map((req, i) => (
              <div key={req.id} className={`${card} p-5`}>
                ...
                <div className={`mt-3 pt-3 border-t ${dk ? "border-white/5" : "border-slate-100"} flex items-center gap-4 flex-wrap`}>
                  <span className={`inline-flex items-center gap-1 text-[11px] font-medium ${dk ? "text-indigo-400" : "text-indigo-600"}`}>
                    <Handshake className="w-3 h-3" />{req.specialization}
                  </span>
                  <span className={`inline-flex items-center gap-1 text-[11px] font-medium ${dk ? "text-white/50" : "text-slate-500"}`}>
