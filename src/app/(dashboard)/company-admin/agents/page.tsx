...
function AgentDetailDrawer({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "seafarers" | "commission"
  >("overview");

  const bg = dk
    ? "bg-[#0d1f35] border-l border-white/8"
  ...
  const tabActive = dk
    ? "bg-[#0d1f35] text-white shadow-sm border border-white/8"
  ...
  const statusCls: Record<string, string> = {
    Active: dk ? "text-emerald-400" : "text-emerald-700",
    Inactive: dk ? "text-slate-400" : "text-slate-500",
    Suspended: dk ? "text-red-400" : "text-red-600",
  };
  const sfStatusCls: Record<string, string> = {
    Active: dk ? "text-emerald-400" : "text-emerald-700",
    Pending: dk ? "text-amber-400" : "text-amber-700",
    Dropped: dk ? "text-red-400" : "text-red-700",
  };
  ...
  const paidCommission = paidSeafarers.reduce(
    (a, s) => a + s.commissionAmount,
    0,
  );
  const pendingComm = unpaidSeafarers.reduce(
    (a, s) => a + s.commissionAmount,
    0,
  );
      ...
      >
        ...
        <div className={`px-5 py-5 border-b ${divBor}`}>
          <div className="flex items-center gap-4">
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center text-lg font-bold text-white shrink-0 ${AVATAR_COLORS[idx % AVATAR_COLORS.length]}`}
            >
              {initials(agent.name)}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span
                  className={`inline-flex items-center gap-1 text-[11px] font-semibold ${statusCls[agent.status]}`}
                >
                  {agent.status === "Active" ? (
                ...
                <span
                  className={`flex items-center gap-1 text-[11px] font-medium ${mt}`}
                >
                  <Star
                    className={`w-3 h-3 ${dk ? "text-amber-400" : "text-amber-500"} fill-current`}
                  />
                  {agent.rating}
                </span>
              </div>
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            {[
              ...
              {
                label: "Disbursed",
                value: formatINR(agent.totalDisbursed),
                color: dk ? "text-sky-400" : "text-sky-600",
              },
            ].map((stat) => (
              <div
                key={stat.label}
                className={`rounded-xl p-3 text-center ${cardBg}`}
              >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className={`rounded-xl p-4 space-y-3 ${cardBg}`}>
                  {[
                    ...
                    },
                    {
                      ...
                      value: `${agent.activeSeafarers} / ${agent.totalSeafarers}`,
                    },
                    ...
                    <div key={label} className="flex items-start gap-3">
                      ...
                      <div>
                        <p
                          className={`text-[10px] uppercase tracking-wide font-medium ${mt}`}
                        >
                          {label}
                        </p>
                        <p className={`text-[13px] font-medium mt-0.5 ${ht}`}>
                          {value}
                        </p>
                      </div>
                    </div>
          ...
          {activeTab === "seafarers" && (
            <div>
              ...
              {seafarers.length === 0 ? (
                <div
                  className={`flex flex-col items-center justify-center py-12 ${mt}`}
                >
                    ...
                    <div key={sf.id} className={`rounded-xl p-3.5 ${cardBg}`}>
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-white shrink-0 ${AVATAR_COLORS[i % AVATAR_COLORS.length]}`}
                          >
                            {initials(sf.name)}
                          </div>
                          <div className="min-w-0">
                            <p
                              className={`text-[13px] font-medium truncate ${ht}`}
                            >
                              {sf.name}
                            </p>
                        ...
                        <span
                          className={`inline-flex items-center gap-1 text-[11px] font-semibold shrink-0 ${sfStatusCls[sf.status]}`}
                        >
                          {sf.status}
                        </span>
                      </div>
                      <div
                        className={`mt-2.5 pt-2.5 border-t ${divBor} grid grid-cols-2 gap-2`}
                      >
                        <div>
                          <p className={`text-[10px] ${mt}`}>Course</p>
                          <p className={`text-[11px] font-medium mt-0.5 ${ht}`}>
                            {sf.course}
                          </p>
                        </div>
                        <div>
                ...
                {[
                  {
                    ...
                    color: dk ? "text-white/80" : "text-slate-800",
                    icon: Wallet,
                  },
                  {
                    label: "Disbursed",
                    value: formatINR(paidCommission),
                    color: dk ? "text-emerald-400" : "text-emerald-600",
                    icon: CheckCircle2,
                  },
                  {
                    label: "Pending",
                    value: formatINR(pendingComm),
                  ...
                  <div key={label} className={`rounded-xl p-3.5 ${cardBg}`}>
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <p
                        className={`text-[10px] font-medium uppercase tracking-wide ${mt}`}
                      >
                        {label}
                      </p>
                    </div>
                    ...
                    <div key={sf.id} className={`rounded-xl p-3.5 ${cardBg}`}>
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p
                              className={`text-[12px] font-medium truncate ${ht}`}
                            >
                              {sf.name}
                            </p>
                            <p className={`text-[10px] ${mt}`}>
                              Fee: {formatINR(sf.feeCollected)}
                            </p>
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          <p
                            className={`text-[12px] font-bold ${sf.commissionPaid ? (dk ? "text-emerald-400" : "text-emerald-600") : dk ? "text-amber-400" : "text-amber-600"}`}
                          >
                            {formatINR(sf.commissionAmount)}
                          </p>
                          <span
                            className={`inline-flex items-center gap-1 text-[11px] font-semibold mt-0.5 ${sf.commissionPaid ? (dk ? "text-emerald-400" : "text-emerald-700") : dk ? "text-amber-400" : "text-amber-700"}`}
                          >
                            {sf.commissionPaid ? (
...
function RequestReviewModal({
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative w-full max-w-md rounded-2xl shadow-2xl ${bg}`}
        onClick={(e) => e.stopPropagation()}
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-[11px] ${mt} mt-0.5`}>
                Applied: {req.requestDate}
              </p>
            </div>
          </div>
          ...
          <div>
            <p
              className={`text-[10px] font-semibold uppercase tracking-widest mb-1.5 ${mt}`}
            >
              Message from Applicant
            </p>
            <p
              className={`text-[13px] leading-relaxed p-3 rounded-xl ${cardBg} ${ht}`}
            >
              {req.message}
            </p>
          </div>
...
export default function AgentsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {toastMsg && (
        <div
          className="fixed top-5 right-5 z-[100] flex items-center gap-2 px-4 py-3 rounded-xl bg-slate-900 text-white text-sm font-medium shadow-xl"
          style={{ animation: "fadeScaleIn 0.2s ease" }}
        >
      ...
      <div className={card}>
        <div
          className={`grid grid-cols-4 divide-x ${dk ? "divide-white/[0.05]" : "divide-slate-100"}`}
        >
          {[
            {
              ...
              <div className="flex items-center gap-4 flex-wrap py-1">
                ...
                {["All", "Active", "Inactive", "Suspended"].map((s) => (
                  <button
                    ...
                    className={`text-xs transition-all cursor-pointer ${statusFilter === s ? filterActive : filterInactive}`}
                  >
                    {s}
                  </button>
                ))}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-11 h-11 rounded-xl flex items-center justify-center text-sm font-bold text-white ${AVATAR_COLORS[i % AVATAR_COLORS.length]}`}
                      >
                        {initials(agent.name)}
                      </div>
                      <div className="min-w-0">
                        <p className={`text-sm font-semibold truncate ${ht}`}>
                          {agent.name}
                        </p>
                        <p className={`text-[11px] truncate ${mt}`}>
                          {agent.company}
                        </p>
                      </div>
                    </div>
                    <span
                      className={`inline-flex items-center gap-1 text-[11px] font-semibold shrink-0 ${statusCls[agent.status]}`}
                    >
                      {agent.status}
                    </span>
                  </div>
                  ...
                  {/* Stats */}
                  <div
                    className={`grid grid-cols-2 gap-2 mb-4 p-3 rounded-xl ${dk ? "bg-white/[0.03] border border-white/5" : "bg-slate-50 border border-slate-100"}`}
                  >
                    <div className="text-center">
                      <p
                        className={`text-base font-bold ${dk ? "text-indigo-400" : "text-indigo-600"}`}
                      >
                        {agent.totalSeafarers}
                      </p>
                      <p className={`text-[10px] ${mt}`}>Seafarers</p>
                    </div>
                    <div className="text-center">
                      <p
                        className={`text-base font-bold ${dk ? "text-emerald-400" : "text-emerald-600"}`}
                      >
                        {formatINR(agent.totalDisbursed)}
                      </p>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span
                        className={`flex items-center gap-1 text-[11px] ${mt}`}
                      >
                        <MapPin className="w-3 h-3" />
                        {agent.location}
                      </span>
                      <span
                        className={`flex items-center gap-1 text-[11px] ${dk ? "text-amber-400" : "text-amber-500"}`}
                      >
                    ...
                    <span
                      className={`flex items-center gap-1 text-[11px] font-medium ${dk ? "text-sky-400 group-hover:text-sky-300" : "text-sky-600 group-hover:text-sky-700"} transition-colors`}
                    >
                  ...
                  {agent.pendingCommission > 0 && (
                    <div
                      className={`mt-3 pt-3 border-t ${dk ? "border-white/5" : "border-slate-100"} flex items-center justify-between`}
                    >
                      <span
                        className={`text-[11px] font-medium flex items-center gap-1 ${dk ? "text-amber-400" : "text-amber-600"}`}
                      >
                        <IndianRupee className="w-3 h-3" />
                        Pending commission
                      </span>
                      <span
                        className={`text-[12px] font-bold ${dk ? "text-amber-400" : "text-amber-600"}`}
                      >
                        {formatINR(agent.pendingCommission)}
                      </span>
                    </div>
                  )}
                </div>
            ...
            requests.map((req, i) => (
              <div key={req.id} className={`${card} p-5`}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold text-white shrink-0 ${AVATAR_COLORS[i % AVATAR_COLORS.length]}`}
                    >
                      {initials(req.name)}
                    </div>
                    <div>
                      <p className={`text-[14px] font-semibold ${ht}`}>
                        {req.name}
                      </p>
                ...
                <div
                  className={`mt-3 pt-3 border-t ${dk ? "border-white/5" : "border-slate-100"} flex items-center gap-4 flex-wrap`}
                >
                  <span
                    className={`inline-flex items-center gap-1 text-[11px] font-medium ${dk ? "text-indigo-400" : "text-indigo-600"}`}
                  >
                    <Handshake className="w-3 h-3" />
                    {req.specialization}
                  </span>
                  <span
                    className={`inline-flex items-center gap-1 text-[11px] font-medium ${dk ? "text-white/50" : "text-slate-500"}`}
                  >
                    <Star className="w-3 h-3" />
                    {req.experience}
                  </span>
                  <span className={`text-[11px] ${mt} ml-auto`}>
                    {req.email}
                  </span>
                </div>
                ...
                {/* Message preview */}
                <div
                  className={`mt-3 p-3 rounded-xl text-[12px] leading-relaxed ${dk ? "bg-white/[0.03] text-white/50 border border-white/5" : "bg-slate-50 text-slate-500 border border-slate-100"}`}
                >
                  {req.message}
                </div>
              </div>
