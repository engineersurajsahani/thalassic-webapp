...
export default function PaymentsPage() {
  ...
  const totalReceived = mockPayments.reduce(
    (s, p) => s + (p.amountReceived ?? (p.status === "Paid" ? p.amount : 0)),
    0,
  );
  const totalPending = mockPayments
    .filter((p) => p.status === "Pending" || p.status === "Partial")
    .reduce((s, p) => s + (p.amount - (p.amountReceived || 0)), 0);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className={`text-sm mt-0.5 ${mt}`}>
          Track course fee collections, invoice records, and payment receipts
        </p>
      </div>
      ...
      <div className={card}>
          ...
          <table className="w-full text-xs">
            <thead>
              ...
              >
                {[
                  ...
                  <th
                    key={h}
                    className={`text-left px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap ${mt}`}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className={`divide-y ${divider}`}>
              {paginatedPayments.map((p) => {
                const received =
                  p.amountReceived ?? (p.status === "Paid" ? p.amount : 0);
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`px-5 py-3.5 max-w-[150px] truncate ${mt}`}>
                      {p.course}
                    </td>
                    <td
                      className={`px-5 py-3.5 max-w-[150px] truncate font-medium ${ht}`}
                    >
                      {p.instituteName}
                    </td>
                    <td className={`px-5 py-3.5 font-bold ${ht}`}>
                    ...
                    <td className="px-5 py-3.5">
                      <span
                        className={`inline-flex items-center gap-1 text-[11px] font-semibold ${dk ? sm.dark : sm.light}`}
                      >
                        {sm.icon}
                        {p.status}
                      </span>
                    ...
                    <td className="px-5 py-3.5">
                      <button
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-semibold border transition-all cursor-pointer ${
                          dk
                            ? "border-white/10 hover:bg-white/10 text-sky-400"
        ...
        {totalPages > 1 && (
          <div
            className={`px-5 py-3 border-t flex items-center justify-between text-xs ${dk ? "border-white/5" : "border-slate-100"}`}
          >
            <span className={dk ? "text-white/40" : "text-slate-400"}>
              Showing page <strong>{currentPage}</strong> of{" "}
              <strong>{totalPages}</strong> ({filtered.length} results)
            </span>
              ...
              <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <div>
                  ...
                  <span className="font-semibold text-sky-400">
                    {selectedInvoicePayment.purchaseType || "HOC"} (Direct)
                  </span>
                </div>
              </div>
