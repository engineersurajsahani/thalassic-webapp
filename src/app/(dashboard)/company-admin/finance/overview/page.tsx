...
import {
  IndianRupee, TrendingUp, Clock, CheckCircle2, AlertCircle,
...
export default function FinanceOverviewPage() {
  ...
  const selectCls = `py-1.5 px-2.5 text-[11px] rounded-lg border outline-none font-medium cursor-pointer ${dk ? "bg-white/5 border-white/10 text-white/70" : "bg-slate-50 border-slate-200 text-slate-600"}`;
  ...
  const stats = useMemo(() => {
    ...
    const overdueAmount  = mockPayments.filter(p => p.status === "Overdue").reduce((s, p) => s + p.amount, 0);
    ...
    return { totalRevenue, totalReceived, pendingAmount, overdueAmount, totalPayments, pendingPayments, monthlyRevenue };
  }, []);

  const kpis = [
    { label: "Total Revenue",       value: stats.totalRevenue,   fmt: "currency", icon: IndianRupee, color: "#6366f1", bg: dk ? "bg-indigo-500/15" : "bg-indigo-50" },
    { label: "Monthly Revenue",     value: stats.monthlyRevenue, fmt: "currency", icon: TrendingUp,  color: "#10b981", bg: dk ? "bg-emerald-500/15": "bg-emerald-50"},
    { label: "Amount Received",     value: stats.totalReceived,  fmt: "currency", icon: CheckCircle2,color: "#22c55e", bg: dk ? "bg-green-500/15"  : "bg-green-50"  },
    { label: "Pending Amount",      value: stats.pendingAmount,  fmt: "currency", icon: Clock,       color: "#f59e0b", bg: dk ? "bg-amber-500/15"  : "bg-amber-50"  },
    { label: "Total Payments",      value: stats.totalPayments,  fmt: "count",    icon: CreditCard,  color: "#3b82f6", bg: dk ? "bg-blue-500/15"   : "bg-blue-50"   },
    { label: "Pending Payments",    value: stats.pendingPayments,fmt: "count",    icon: AlertCircle, color: "#f43f5e", bg: dk ? "bg-rose-500/15"   : "bg-rose-50"   },
  ];
  ...
  const byInstitute = useMemo(() => {
    const filtered = revenueInstFilter === "All"
      ? mockPayments
      : mockPayments.filter(p => p.instituteName === revenueInstFilter);
    const map: Record<string, { name: string; Received: number; Pending: number }> = {};
    filtered.forEach(p => {
      if (!map[p.instituteId]) {
        map[p.instituteId] = {
          name: p.instituteName.length > 20 ? p.instituteName.slice(0, 20) + "…" : p.instituteName,
  ...
  const collectionStats = useMemo(() => {
    const filtered = collectionInstFilter === "All"
      ? mockPayments
      : mockPayments.filter(p => p.instituteName === collectionInstFilter);
    const received = filtered.filter(p => p.status === "Paid").reduce((s, p) => s + p.amount, 0);
    ...
    return { received, pending, overdue, total, paidCount, pendCount, overdueCount };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className={`text-sm mt-0.5 ${mt}`}>Financial analytics, invoices, and payment tracking for seafarer training</p>
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        {kpis.map(k => {
          const Icon = k.icon;
          return (
            <div key={k.label} className={`${card} flex items-center gap-4 px-6 py-5`}>
              ...
              <div>
                <p className={`text-2xl font-bold ${ht}`}>{fmt(k.value, k.fmt)}</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                ...
                {instituteNames.map(name => (
                  <option key={name} value={name}>{name.length > 30 ? name.slice(0, 30) + "…" : name}</option>
                ))}
              </select>
            </div>
          </div>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={byInstitute} barGap={4} barCategoryGap="20%">
                  ...
                  <XAxis
                    dataKey="name"
                    tick={{ fontSize: 10, fill: dk ? "rgba(255,255,255,0.4)" : "#94a3b8" }}
                    axisLine={false}
                    tickLine={false}
                    interval={0}
                    angle={-20}
                  ...
                  />
                  <Tooltip
                    formatter={(val: any, name: any) => [`₹${Number(val).toLocaleString("en-IN")}`, String(name)]}
                    contentStyle={{
        ...
        <div className={`${card} p-6 flex flex-col justify-between`}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 my-auto">
            ...
            <div className="h-44 w-44 relative shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-[10px] uppercase font-bold text-black dark:text-white opacity-70">Total</span>
            ...
            <div className="flex-1 w-full space-y-3">
              <div className="flex items-center justify-between p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-black dark:text-white">Amount Received</span>
                </div>
              ...
              <div className="flex items-center justify-between p-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-black dark:text-white">Amount Pending</span>
                </div>
                <span className="text-xs font-bold text-black dark:text-white">
              ...
              <div className="flex items-center justify-between p-2 rounded-lg bg-rose-500/10 border border-rose-500/20">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-black dark:text-white">Amount Overdue</span>
                </div>
                <span className="text-xs font-bold text-black dark:text-white">
          ...
          <div className={`grid grid-cols-3 gap-2 pt-3 border-t text-center ${dk ? "border-white/5" : "border-slate-100"}`}>
            <div>
              <p className="text-base font-black text-black dark:text-white">
                {collectionStats.paidCount}
              </p>
              <p className="text-[10px] font-bold text-black dark:text-white opacity-70">Paid</p>
            </div>
            <div>
              <p className="text-base font-black text-black dark:text-white">
                {collectionStats.pendCount}
              </p>
              <p className="text-[10px] font-bold text-black dark:text-white opacity-70">Pending</p>
            </div>
            <div>
              <p className="text-base font-black text-black dark:text-white">
                {collectionStats.overdueCount}
              </p>
              <p className="text-[10px] font-bold text-black dark:text-white opacity-70">Overdue</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={dk ? "border-b border-white/5" : "border-b border-slate-100"}>
                {["ID", "Seafarer", "Course", "Institute", "Amount", "Status", "Date"].map(h => (
            ...
            <tbody className={`divide-y ${divider}`}>
              {recentTransactions.map(p => (
                <tr key={p.id} className={`transition-colors ${dk ? "hover:bg-white/[0.03]" : "hover:bg-slate-50"}`}>
                  <td className={`px-5 py-3.5 text-[11px] font-mono ${mt}`}>{p.txnId || p.id}</td>
                  <td className={`px-5 py-3.5 text-[13px] font-medium ${ht}`}>{p.seafarerName}</td>
                  <td className={`px-5 py-3.5 text-[12px] max-w-[140px] truncate ${mt}`}>{p.course}</td>
