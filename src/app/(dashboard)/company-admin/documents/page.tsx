...
export default function DocumentVerificationPage() {
  ...
  const [selectedSeafarerId, setSelectedSeafarerId] = useState<string | null>(
    seafarers[0]?.id || null,
  );
  ...
  const [previewingDoc, setPreviewingDoc] = useState<SeafarerDocument | null>(
    null,
  );
  ...
  const filteredPurchases = useMemo(() => {
    return mockDocumentPurchases.filter((p) => {
      ...
      const matchStatus =
        purchaseStatusFilter === "All" || p.status === purchaseStatusFilter;
  ...
  const purchaseStats = useMemo(() => {
    ...
    const completed = mockDocumentPurchases.filter(
      (p) => p.status === "Completed",
    ).length;
    const processing = mockDocumentPurchases.filter(
      (p) => p.status === "Processing",
    ).length;
    const pending = mockDocumentPurchases.filter(
      (p) => p.status === "Pending",
    ).length;
    return {
      total: mockDocumentPurchases.length,
      totalSpent,
      completed,
      processing,
      pending,
      ...
      setSeafarers((prev) =>
        prev.map((sf) => {
          if (sf.id === selectedSeafarerId) {
            return {
              ...sf,
              documents: sf.documents.map((d) =>
                d.id === docId
                  ? { ...d, status: "Approved", rejectionReason: undefined }
                  : d,
              ),
            };
          }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          >
            View, verify seafarer documents, and manage certification purchase
            history.
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            {[
              ...
              {
                label: "Pending",
                val: docStats.pending,
                color: "text-amber-500",
              },
              {
                label: "Approved",
                val: docStats.approved,
                color: "text-emerald-500",
              },
              {
                label: "Rejected",
                val: docStats.rejected,
                color: "text-rose-500",
              },
              {
                label: "Expired",
                val: docStats.expired,
                color: "text-gray-500",
              },
            ].map((st, idx) => (
              <div
                key={idx}
                className={`p-3 rounded-lg border ${isDark ? "bg-[#0c1a2e] border-white/5" : "bg-white border-slate-200"}`}
              >
                <p className="text-[9px] font-bold uppercase tracking-wider opacity-45">
                  {st.label}
                </p>
                <p className={`text-base font-black mt-0.5 ${st.color}`}>
                  {st.val}
                </p>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            >
              <div
                className={`p-4 border-b ${isDark ? "border-white/5" : "border-slate-100"}`}
              >
              ...
              <div className="flex-1 overflow-y-auto">
                {filteredSeafarers.map((sf) => {
                  ...
                  const pendingCount = sf.documents.filter(
                    (d) => d.status === "Pending",
                  ).length;
                  return (
                    <button
                    ...
                    >
                      <div className="flex items-center gap-3 text-left">
                        ...
                        <div>
                          ...
                          <p
                            className={`text-[10px] ${isDark ? "text-white/50" : "text-slate-500"}`}
                          >
                            {sf.rank}
                          </p>
                        </div>
            ...
            <div
              className={`lg:col-span-2 rounded-xl border flex flex-col h-[600px] ${
                isDark
                  ? "bg-[#0c1a2e] border-white/5"
                  : "bg-white border-slate-200"
              }`}
            >
              {selectedSeafarer ? (
                <>
                  {/* Profile Header */}
                  <div
                    className={`p-6 border-b flex items-start gap-4 ${isDark ? "border-white/5" : "border-slate-100"}`}
                  >
                    ...
                    <div>
                      ...
                      <div
                        className={`flex items-center gap-3 text-[11px] mt-1 ${isDark ? "text-white/60" : "text-slate-500"}`}
                      >
                        ...
                        <span>•</span>
                        <span>
                          INDOS: {selectedSeafarer.indosNumber || "N/A"}
                        </span>
                      </div>
                    </div>
                  </div>
                              ...
                              <>
                                ...
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex items-start gap-3">
                                    <div
                                      className={`p-2 rounded-lg ${isDark ? "bg-white/5" : "bg-slate-100"}`}
                                    >
                                    ...
                                    <div>
                                      ...
                                      <p
                                        className={`text-xs mt-0.5 ${isDark ? "text-white/50" : "text-slate-500"}`}
                                      >
                                        Type: {doc.type}
                                      </p>
                                      <p
                                        className={`text-[10px] mt-0.5 ${isDark ? "text-white/40" : "text-slate-400"}`}
                                      >
                                        Validity: {doc.issueDate} to{" "}
                                        {doc.expiryDate}
                                      </p>
                                    </div>
                                  </div>
                                ...
                                {/* Actions Footer */}
                                <div
                                  className={`pt-3 border-t flex items-center justify-between gap-3 ${isDark ? "border-white/10" : "border-slate-100"}`}
                                >
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
                  ...
                  >
                    Select a seafarer to view their documents
                  </p>
                </div>
              )}
            </div>
      ...
      {activeTab === "purchase-history" && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div
              className={`p-4 rounded-xl border ${isDark ? "bg-[#0c1a2e] border-white/5" : "bg-white border-slate-200 shadow-sm"}`}
            >
              <p className="text-[10px] font-bold uppercase tracking-wider opacity-45">
                Total Purchases
              </p>
              <p
            ...
            >
              ...
              <p className="text-xl font-bold mt-1 text-emerald-500">
                ₹{purchaseStats.totalSpent.toLocaleString("en-IN")}
              </p>
            </div>
            <div
              className={`p-4 rounded-xl border ${isDark ? "bg-[#0c1a2e] border-white/5" : "bg-white border-slate-200 shadow-sm"}`}
            >
              ...
              <p className="text-xl font-bold mt-1 text-sky-500">
            ...
            >
              ...
              <p className="text-xl font-bold mt-1 text-amber-500">
                {purchaseStats.processing + purchaseStats.pending}
              </p>
            </div>
          </div>
          ...
          >
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead>
                  <tr
                    className={`border-b text-[10px] font-bold uppercase tracking-wider opacity-60 ${
                      isDark
                ...
                >
                  {paginatedPurchases.length === 0 ? (
                    <tr>
                      <td
                        colSpan={7}
                        className="px-5 py-10 text-center opacity-40"
                      >
                    ...
                    paginatedPurchases.map((rec) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <button
                            onClick={() =>
                              handleDownload(`${rec.documentName} Receipt.pdf`)
                            }
            ...
            {totalPurchasePages > 1 && (
              ...
              >
                <span className="opacity-50">
                  Showing {(purchasePage - 1) * purchasesPerPage + 1}–
                  {Math.min(
                    purchasePage * purchasesPerPage,
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setPurchasePage((p) =>
                        Math.min(totalPurchasePages, p + 1),
                      )
                    }
                    className={`flex items-center gap-1 px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed ${
                      isDark
          ...
          >
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold">
                  Document Preview: {previewingDoc.name}
                </h3>
              </div>
              <button
                onClick={() => setShowPreviewModal(false)}
            ...
            <div className="p-6 flex flex-col items-center justify-center min-h-[300px] bg-slate-100 dark:bg-[#080d16] text-center border-b border-solid border-slate-200 dark:border-white/5 relative overflow-hidden group">
              {/* Scan effect lines */}
              <div
                className="absolute inset-x-0 top-0 h-0.5 bg-sky-500 opacity-60 shadow-lg animate-scan"
                style={{ animation: "scan 3.5s linear infinite" }}
              />

              <div
                className={`p-6 rounded-2xl border border-dashed border-sky-400/50 bg-sky-500/5 max-w-md space-y-4`}
              >
                <FileText className="w-12 h-12 text-sky-400 mx-auto" />
                <div>
                  <h4 className="font-bold text-xs">{previewingDoc.name}</h4>
                  <p className="text-[10px] text-gray-500 mt-1">
                    Submitted by {selectedSeafarer.name} (
                    {selectedSeafarer.rank})
                  </p>
                </div>
                <div
                  className={`text-[10px] py-2 px-3 bg-white/5 rounded border text-left space-y-1 font-mono opacity-80 ${
                ...
                <span className="text-[9px] opacity-45 italic block">
                  Interactive scan and PDF visual stream rendering is a backend
                  simulator mockup.
                </span>
              </div>
            </div>
            ...
            <div
              className={`p-4 flex items-center justify-between gap-4 ${isDark ? "bg-[#09111e]" : "bg-slate-50"}`}
            >
              <div className="flex gap-2">
