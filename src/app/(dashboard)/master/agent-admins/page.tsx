...
type ModalType =
  "view" | "add" | "reset" | "login" | "permissions" | "audit" | null;
...
export default function PartnerAdminsPage() {
  ...
  const [viewTab, setViewTab] = useState<
    "profile" | "seafarers" | "courses" | "settlements"
  >("profile");
  ...
  const inputCls = dk
    ? "bg-white/5 border border-white/8 text-white placeholder:text-slate-400 focus:border-violet-500/50 outline-none"
    : "bg-slate-50 border border-slate-200 text-slate-800 placeholder:text-black focus:border-violet-400 outline-none";
  ...
  const associatedSeafarers = selected
    ? MOCK_SEAFARERS.filter(
        (s) =>
          s.sourceType === "Partner" &&
          s.sourceName.toLowerCase().includes(selected.name.toLowerCase()),
      )
    : [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`text-sm mt-0.5 ${mt}`}>
            Manage registered maritime recruitment partners, RPSL agencies, and
            course assignments
          </p>
        </div>
        <button
        ...
        {[
          {
            ...
            val: partnerList.length,
            sub: "Registered agencies",
            Icon: Handshake,
            c: "text-violet-400",
          ...
          },
          {
            ...
            val: partnerList.filter((p) => p.status === "active").length,
            sub: "Operational",
            Icon: CheckCircle2,
            c: "text-emerald-400",
          ...
          },
          {
            ...
            val: partnerList.reduce((acc, p) => acc + p.totalSeafarers, 0),
            sub: "Associated crew",
            Icon: Users,
            c: "text-sky-400",
          ...
          },
          {
            ...
            val: `₹${(partnerList.reduce((acc, p) => acc + p.pendingAmount, 0) / 1000).toFixed(0)}K`,
            sub: "To be received",
            Icon: Wallet,
            c: "text-rose-400",
            bg: dk ? "bg-rose-500/10" : "bg-rose-50",
          },
        ].map((k) => (
          <div key={k.label} className={`${card} p-4 flex items-center gap-3`}>
            ...
            <div>
              ...
              <p className={`text-xs font-semibold mt-0.5 ${ht} opacity-75`}>
                {k.label}
              </p>
      ...
      >
        <div className="relative flex-1 min-w-[240px]">
          <Search
            className={`w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 ${mt}`}
          />
          <input
            value={search}
            onChange={(e) => {
      ...
      <div className={card}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className={`divide-y ${dv}`}>
              {paginatedPartners.map((p) => {
                ...
                const SIcon =
                  (statusItem && STATUS_ICON_MAP[statusItem.iconName]) ||
                  CheckCircle2;
                return (
                  <tr
                    key={p.id}
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`text-[12px] font-mono font-medium text-black dark:text-white`}
                      >
                        {p.rpslNumber}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="space-y-0.5">
                        <p className={`text-[12px] font-medium ${ht}`}>
                          {p.contactPerson}
                        </p>
                        <p
                          className={`text-[11px] flex items-center gap-1 ${mt}`}
                        >
                    ...
                    <td className="px-6 py-4 text-[11px]">
                      <span className="text-emerald-500 font-semibold">
                        ₹{(p.totalAmountReceived / 1000).toFixed(0)}K
                      </span>
                      <span className={`mx-1 ${mt}`}>/</span>
                      <span className="text-rose-500 font-semibold">
                        ₹{(p.pendingAmount / 1000).toFixed(0)}K
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span
        ...
        >
          ...
          <span className={`text-[11px] ${mt}`}>
            Compliant with PRD §1.8 (View-Only Management)
          </span>
        </div>
      </div>
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`inline-flex items-center gap-1 text-[11px] font-medium ${getStatus(selected.status)?.color || "text-emerald-500"}`}
                    >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs">
              ...
              {viewTab === "profile" && (
                <div className="space-y-6">
                    ...
                    >
                      ...
                      <div className="space-y-3 text-xs">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span
                            className={`inline-flex items-center gap-1 font-semibold ${getStatus(selected.status)?.color || "text-emerald-500"}`}
                          >
                            {getStatus(selected.status)?.label ||
                              selected.status}
                          </span>
                  ...
                  <div
                    className={`p-4 rounded-xl border ${dk ? "bg-white/[0.02] border-white/5" : "bg-slate-50 border-slate-200"}`}
                  >
                    <h3 className="text-xs font-bold uppercase tracking-wider mb-3 text-slate-700 dark:text-slate-300">
                      Financial & Settlement Overview
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        <p className="text-[10px] font-semibold uppercase text-slate-600 dark:text-slate-400">
                          Total Amount Payable
                        </p>
                        <p className="text-lg font-bold mt-1 text-slate-900 dark:text-slate-100">
                          ₹{selected.totalAmountPayable.toLocaleString()}
                        </p>
                        <p className="text-[11px] mt-0.5 text-slate-600 dark:text-slate-400 font-medium">
                          Configured Hari Om Pricing
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] font-semibold uppercase text-slate-600 dark:text-slate-400">
                        ...
                        <p className="text-lg font-bold mt-1 text-emerald-600 dark:text-emerald-400">
                          ₹{selected.totalAmountReceived.toLocaleString()}
                        </p>
                        <p className="text-[11px] mt-0.5 text-slate-600 dark:text-slate-400 font-medium">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold mt-1 text-rose-500 dark:text-rose-400">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                      ...
                      <div>
                        <p className="text-xl font-bold text-slate-900 dark:text-slate-100">
                          ₹{(selected.totalAmountPayable / 1000).toFixed(0)}K
                        </p>
                        <p className="text-[11px] text-slate-600 dark:text-slate-400 font-medium mt-0.5">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                      Associated Seafarers ({associatedSeafarers.length})
                    </p>
                    <span className="text-[11px] text-slate-600 dark:text-slate-400 font-medium">
                      Registered via {selected.name}
                    </span>
                  </div>
                  {associatedSeafarers.length > 0 ? (
                    <div
                      className={`border rounded-xl divide-y ${dk ? "border-white/5 divide-white/5" : "border-slate-200 divide-slate-100"}`}
                    >
                        ...
                        >
                          <div>
                            ...
                            <p className="text-[11px] font-mono text-slate-600 dark:text-slate-400 mt-0.5">
                              INDOS: {s.indosNumber} · CDC: {s.cdcNumber}
                            </p>
                          </div>
                          <div className="text-right">
                            ...
                            <p className="text-[10px] text-slate-600 dark:text-slate-400 mt-0.5">
                              {s.enrollments.length} Course Enrollments
                            </p>
                          </div>
                        </div>
                      ))}
              ...
              {viewTab === "courses" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                      Assigned Course Pricing Schedule
                    </p>
                    <span className="text-[11px] text-slate-600 dark:text-slate-400 font-medium">
                      Amount Payable to Hari Om
                    </span>
                  </div>
                  <div
                    className={`border rounded-xl overflow-hidden ${dk ? "border-white/5" : "border-slate-200"}`}
                  >
                    <table className="w-full text-left">
                      <thead
                        className={`text-[10px] font-semibold uppercase ${dk ? "bg-white/5 text-slate-400" : "bg-slate-100 text-slate-600"}`}
                      >
                        <tr>
                      ...
                      <tbody
                        className={`divide-y ${dk ? "divide-white/5" : "divide-slate-100"}`}
                      >
                        {selected.assignedPricing.map((ap) => (
                          <tr key={ap.courseId}>
                            <td className={`p-3 font-medium ${ht}`}>
                              {ap.courseTitle}
                            </td>
                            <td className="p-3 font-bold text-emerald-500">
                              ₹{ap.hariomPrice.toLocaleString()}
                            </td>
                            <td className="p-3 text-slate-700 dark:text-slate-300 font-medium">
                              ₹{ap.suggestedSellingPrice.toLocaleString()}
                            </td>
                            <td className="p-3">
              ...
              {viewTab === "settlements" &&
                (() => {
                  const s = MOCK_PARTNER_SETTLEMENTS.find(
                    (item) => item.partnerId === selected.id,
                  ) || {
                    ...
                    settlementStatus:
                      selected.pendingAmount === 0
                        ? "Settled"
                        : selected.totalAmountReceived > 0
                          ? "Partially Settled"
                          : "Pending",
                  ...
                  const statusColor =
                    ...
                    s.settlementStatus === "Partially Settled"
                  ...
                  return (
                    <div className="space-y-4">
                      <div
                        className={`p-4 rounded-xl border ${dk ? "bg-white/[0.02] border-white/5" : "bg-slate-50 border-slate-200"}`}
                      >
                        <h3 className="text-xs font-bold uppercase tracking-wider mb-4 text-slate-700 dark:text-slate-300">
                          Settlement Details
                        </h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          ...
                          <div>
                            ...
                            <p
                              className={`text-sm font-mono font-medium mt-1 ${dk ? "text-white/80" : "text-slate-700"}`}
                            >
                        ...
                        s.purchases.length > 0 && (
                          ...
                          >
                            <h4 className="text-xs font-bold uppercase tracking-wider mb-3 text-slate-700 dark:text-slate-300">
                              Purchases in Settlement ({s.id})
                            </h4>
                            <div
                              className={`border rounded-lg overflow-hidden ${dk ? "border-white/5" : "border-slate-200"}`}
                            >
                              <table className="w-full text-left text-xs">
                                ...
                                >
                                  {s.purchases.map((p, idx) => (
                                    <tr key={idx}>
                                      <td className={`p-2.5 font-medium ${ht}`}>
                                        {p.seafarerName}
                                      </td>
            ...
            >
              <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">
                RPSL: {selected.rpslNumber} · Partner ID: {selected.id} ·
      ...
      {modal === "add" && (
        ...
        >
          <div
            className={`w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden p-6 space-y-4 ${modalBg}`}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
              <h2 className={`text-base font-bold ${ht}`}>
            ...
            <div className="space-y-3">
              <div>
                <label className={`text-[11px] block mb-1 ${labelCls}`}>
                  Partner Name / Brand *
                </label>
                <input
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className={`w-full px-3 py-2 text-xs rounded-xl ${inputCls}`}
                  placeholder="e.g. Ocean Maritime Recruitment"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 text-xs rounded-xl ${inputCls}`}
                    placeholder="e.g. Ocean Maritime Services Pvt Ltd"
                  />
                </div>
                <div>
                  <label className={`text-[11px] block mb-1 ${labelCls}`}>
                  ...
                  <input
                    value={form.rpslNumber}
                    onChange={(e) =>
                      setForm({ ...form, rpslNumber: e.target.value })
                    }
                    className={`w-full px-3 py-2 text-xs rounded-xl ${inputCls}`}
                    placeholder="e.g. RPSL-MUM-482"
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                onClick={closeModal}
                className={`px-4 py-2 text-xs font-semibold rounded-xl border transition-colors ${dk ? "bg-white/5 border-white/10 text-white/80 hover:bg-white/10" : "bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200"}`}
              >
                Cancel
              </button>
              <button
                onClick={handleAddPartner}
                className="px-5 py-2 text-xs font-semibold rounded-xl bg-violet-500 hover:bg-violet-600 text-white transition-colors"
              >
      ...
      {modal === "reset" && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div
            className={`w-full max-w-sm rounded-2xl shadow-2xl p-6 text-center space-y-4 ${modalBg}`}
          >
            ...
            {resetDone ? (
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-xs font-semibold rounded-xl bg-amber-500 hover:bg-amber-600 text-white"
                >
                  Send Reset Email
                </button>
              </div>
      ...
      {modal === "audit" && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div
            className={`w-full max-w-md rounded-2xl shadow-2xl p-6 space-y-4 ${modalBg}`}
          >
            <div className="flex items-center justify-between">
              <h2 className={`text-sm font-bold ${ht}`}>
                Partner Audit Log · {selected.name}
              </h2>
              <button onClick={closeModal} className={mt}>
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="space-y-2.5 max-h-64 overflow-y-auto">
              {AUDIT_LOG.map((al, idx) => (
                <div
                  key={idx}
                  className={`p-2.5 rounded-xl border text-xs ${dk ? "bg-white/3 border-white/5" : "bg-slate-50 border-slate-100"}`}
                >
                  <div className="flex items-center justify-between">
            ...
            <div className="flex justify-end">
              <button
                onClick={closeModal}
                className={`px-4 py-1.5 text-xs rounded-xl ${dk ? "bg-white/10 text-white" : "bg-slate-200 text-slate-700"}`}
              >
                Close
              </button>
            </div>
