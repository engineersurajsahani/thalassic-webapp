...
import {
  ...
  Download, UserCheck, UserX, Info,
} from "lucide-react";

// Master manages ONLY Company Admins and Agent Admins
// Seafarers are managed within Company Admin / Agent Admin portals
const ACCOUNTS = [
  // Company Admins
...
export default function UsersPage() {
  ...
  const companyCount = ACCOUNTS.filter(u => u.role === "company_admin").length;
  const agentCount   = ACCOUNTS.filter(u => u.role === "agent_admin").length;
  const activeCount  = ACCOUNTS.filter(u => u.status === "active").length;
  const pendingCount = ACCOUNTS.filter(u => u.status === "pending").length;
