                          ...
                          {openDropdown === a.id && (
                            <>
                              ...
                              <div className={`absolute right-0 mt-1 w-48 rounded-xl shadow-xl border z-20 py-1 ${dk ? "bg-[#0c1a2e] border-white/10" : "bg-white border-slate-200"}`}>
                                <button
                                  onClick={() => { setOpenDropdown(null); openModal("edit", a); }}
                                ...
                                >
                                  <Edit className="w-3.5 h-3.5 text-sky-400" /> Edit Administrator
                                </button>
                                <button
                                  onClick={() => { setOpenDropdown(null); toggleStatus(a); }}
                                  className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold transition-colors ${dk ? "text-white/70 hover:bg-white/8 hover:text-white" : "text-slate-700 hover:bg-slate-50"}`}
                                >
                                  {a.status === "active"
                                    ? <><PowerOff className="w-3.5 h-3.5 text-red-400" /> Deactivate</>
                                    : <><Power className="w-3.5 h-3.5 text-emerald-400" /> Activate</>}
                                </button>
                                <button
                                  onClick={() => { setOpenDropdown(null); openModal("reset", a); }}
                                  className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold transition-colors ${dk ? "text-white/70 hover:bg-white/8 hover:text-white" : "text-slate-700 hover:bg-slate-50"}`}
                                >
                                  <Key className="w-3.5 h-3.5 text-amber-400" /> Reset Password
                                </button>
                                <button
                                  onClick={() => { setOpenDropdown(null); openModal("login", a); }}
                                  className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold transition-colors ${dk ? "text-white/70 hover:bg-white/8 hover:text-white" : "text-slate-700 hover:bg-slate-50"}`}
                                >
                                  <History className="w-3.5 h-3.5 text-indigo-400" /> Login History
                                </button>
                              </div>
                            </>
                          )}
