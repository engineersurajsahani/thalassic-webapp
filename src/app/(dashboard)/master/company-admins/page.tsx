...
export default function CompanyAdminsPage() {
  ...
  const closeModal = () => { setModal(null); setSelected(null); setSaved(false); };
      ...
      <div className={card}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className={thCls}>
                {["Company Admin","Contact","Location","Seafarers","Status","Actions"].map(h => (
                  <th key={h} className={`px-6 py-3 text-[10px] font-semibold uppercase tracking-wider ${h === "Actions" ? "text-right" : "text-left"}`}>{h}</th>
                ))}
              </tr>
            </thead>
                  ...
                  <tr key={a.id} className={`${rh} transition-colors`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`font-semibold text-xs ${ht}`}>{a.name}</p>
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-0.5">
                        <p className={`text-xs flex items-center gap-1.5 ${ht}`}><Mail className="w-3 h-3 text-slate-400" />{a.email}</p>
                        <p className={`text-[11px] flex items-center gap-1.5 ${mt}`}><Phone className="w-3 h-3 text-slate-400" />{a.phone}</p>
                      </div>
                    </td>
                    <td className={`px-6 py-4 text-xs ${mt}`}>
                      <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{a.location}{a.branch ? ` (${a.branch})` : ""}</span>
                    </td>
                    <td className={`px-6 py-4 text-xs font-semibold ${ht}`}>{a.seafarers}</td>
                    <td className="px-6 py-4">
