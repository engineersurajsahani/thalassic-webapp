                      ...
                      <td className="p-4 pr-6 text-right">
                        <div className="relative inline-block">
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${isDark ? "bg-white/5 border-white/10 text-white/70 hover:bg-white/10" : "bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200"}`}
                          >
                          ...
                          {openDropdown === item.id && (
                            <>
                              ...
                              <div className={`absolute right-0 mt-1 w-44 rounded-xl shadow-xl border z-20 py-1 ${isDark ? "bg-[#0c1a2e] border-white/10" : "bg-white border-slate-200"}`}>
                                {isPending ? (
                                  <>
                                    <button
                                      onClick={() => { setOpenDropdown(null); handleApprove(item); }}
                                      disabled={processingId === item.id}
                                      className="w-full flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold text-emerald-400 hover:bg-emerald-500/10 transition-colors disabled:opacity-50"
                                    >
                                      <CheckCircle2 className="w-3.5 h-3.5" /> Approve
                                    </button>
                                    <button
                                      onClick={() => { setOpenDropdown(null); setRejectCourse(item); setRejectReason(""); }}
                                      disabled={processingId === item.id}
                                      className="w-full flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold text-red-400 hover:bg-red-500/10 transition-colors disabled:opacity-50"
                                    >
                                      <XCircle className="w-3.5 h-3.5" /> Reject
                                    </button>
                                  </>
                                ) : (
                                  <p className={`px-3.5 py-2 text-xs ${isDark ? "text-white/30" : "text-slate-400"}`}>No pending action</p>
                                )}
                              </div>
                            </>
                          )}
