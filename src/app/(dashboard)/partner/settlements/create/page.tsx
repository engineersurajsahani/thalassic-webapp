...
export default function SubmitSettlementPage() {
  ...
  const [expectedDueDate, setExpectedDueDate] =
    useState<string>(defaultDueDate);
  ...
  useEffect(() => {
    async function loadPurchases() {
      ...
      try {
        ...
        const [purchasesData, settlementsData] = await Promise.all([
          ...
          partnerService.getSettlements(),
        ]);

        const itemsMap = new Map<string, any>();

        // 1. Process real unsubmitted user purchases
        (purchasesData || []).forEach((p: any) => {
          ...
          // Exclude purchases that are already submitted, under verification, completed, or settled
          const isSubmittedOrDone =
            statusNorm === "submitted" ||
            statusNorm === "under verification" ||
            statusNorm === "under_verification" ||
            statusNorm === "completed" ||
            statusNorm === "settled" ||
            statusNorm === "approved" ||
            (rem === 0 && statusNorm !== "pending");

          if (!isSubmittedOrDone) {
            itemsMap.set(p.id, {
              ...p,
              id: p.id,
              seafarerName:
              ...
              payableAmount: rem > 0 ? rem : Number(p.payableAmount || 14250),
              originalPayableAmount: Number(p.payableAmount || 14250),
              settlementStatus: p.settlementStatus || "Pending",
              isPartialBalance:
                rem > 0 && rem < Number(p.payableAmount || 14250),
            });
          }
        });

        // 2. Process partial/split settlements with remaining pending balance (> ₹0)
        (settlementsData || []).forEach((s: any) => {
          const totAmt = Number(
            s.amount_payable ?? s.total_amount ?? s.totalAmount ?? 0,
          );
          const paidAmt = Number(
            s.amount_settled ?? s.paid_amount ?? s.paidAmount ?? 0,
          );
          ...
          if (pendingAmt > 0) {
            const purchasesList =
              s.related_purchases || s.purchases || s.allocations || [];

            if (purchasesList.length > 0) {
              // Calculate candidate sequential allocations for this settlement
              let cumulativeFees = 0;
              purchasesList.forEach((alloc: any, idx: number) => {
                const itemFee = Number(
                  alloc.payableAmount ??
                    alloc.hariom_payable ??
                    alloc.amount ??
                    14250,
                );

                const prevAllocated = Math.min(paidAmt, cumulativeFees);
                const currentPaid = Math.max(
                  0,
                  Math.min(itemFee, paidAmt - prevAllocated),
                );
                const itemPending = Math.max(0, itemFee - currentPaid);

                cumulativeFees += itemFee;

                if (itemPending > 0) {
                  const allocId =
                    alloc.id ||
                    alloc.purchaseId ||
                    `${s.id || s.settlement_number}-alloc-${idx}`;
                  const sfName =
                    alloc.seafarerName ||
                    alloc.seafarer_name ||
                    alloc.customer_name ||
                  ...
                  itemsMap.set(allocId, {
                    id: allocId,
                    purchaseId: alloc.purchaseId || alloc.id,
                    settlementRef:
                      s.settlement_number ||
                      s.settlementNumber ||
                    ...
                    seafarerName: sfName,
                    courseName: crsName,
                    payableAmount: itemPending,
                    originalPayableAmount: itemFee,
                    settlementStatus: "2nd Installment Balance",
                    isPartialBalance: true,
                  });
                }
              });
            } else {
              const setKey = s.id || s.settlement_number || s.settlementNumber;
              itemsMap.set(setKey, {
                id: setKey,
                settlementRef:
                ...
                seafarerName:
                  s.seafarerName ||
                  s.seafarer_name ||
                  `Settlement ${s.settlement_number || s.id} Balance`,
                courseName:
                  s.courseName ||
                  s.course_name ||
                  "Partial Remittance 2nd Installment Balance",
                payableAmount: pendingAmt,
                originalPayableAmount: totAmt,
                settlementStatus: "2nd Installment Balance",
                isPartialBalance: true,
              });
            }
        ...
        const eligible = Array.from(itemsMap.values());
    ...
    window.addEventListener("focus", loadPurchases);
    window.addEventListener("storage", loadPurchases);
    return () => {
      window.removeEventListener("focus", loadPurchases);
  ...
  const totalAmount = selectedPurchases.reduce(
    (acc, curr) => acc + Number(curr.payableAmount || 0),
    0,
  );
  ...
  const effectivePaidAmount =
    paymentMode === "full"
      ? totalAmount
      : paidAmountInput !== ""
        ? Math.min(totalAmount, Math.max(0, Number(paidAmountInput)))
        : 0;

  const remainingBalance =
    paymentMode === "full" ? 0 : Math.max(0, totalAmount - effectivePaidAmount);
  ...
  const getItemizedAllocations = () => {
    ...
    if (allocationMode === "manual") {
      return selectedPurchases.map((p) => {
        const payable = Number(p.payableAmount || 0);
        const customPaid =
          manualAllocations[p.id] !== undefined
        ...
        statusLabel:
          paidNow === payable
            ...
            : paidNow > 0
              ...
              : "0% Paid",
        badgeColor:
          paidNow === payable ? "emerald" : paidNow > 0 ? "amber" : "gray",
      };
    });
  };
  ...
  const handleManualAllocationChange = (
    ...
    const val =
      valStr === "" ? 0 : Math.min(maxPayable, Math.max(0, Number(valStr)));
  ...
  const createDefaultProofDataUrl = (
    refNum: string,
    date: string,
    amount: number,
    method: string,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMode === "partial") {
      ...
      if (isNaN(parsedPaid) || parsedPaid <= 0) {
        setError(
          "Please enter a valid amount being paid now (greater than ₹0).",
        );
        return;
      }
      if (parsedPaid >= totalAmount) {
        setError(
          "For 100% full payment, please select 'Full Remittance Batch (100%)' mode.",
        );
        return;
      }
      if (!expectedDueDate) {
        setError(
          "Please select an expected due date for the remaining balance payment.",
        );
        return;
      }
    }
    ...
    if (referenceNumber.trim().length > 22) {
      setError(
        "Bank UTR / Remittance reference number cannot exceed 22 characters.",
      );
      return;
    }
    ...
    try {
      ...
      if (bankStatementFile) {
        ...
        proofUrl = createDefaultProofDataUrl(
      ...
      const targetSettlementRef =
        (itemizedAllocations as any[]).find((a) => a.settlementRef)
          ?.settlementRef ||
        (purchases as any[]).find(
          (p) => selectedIds.includes(p.id) && p.settlementRef,
        )?.settlementRef;
      ...
      const res = await partnerService.submitSettlement({
        ...
        expectedDueDate:
          paymentMode === "partial" ? expectedDueDate : undefined,
        totalAmount,
        proofUrl,
        proofFileName,
  ...
  if (createdSettlement) {
    const isPartialSuccess =
      createdSettlement.paymentMode === "partial" ||
      createdSettlement.payment_mode === "partial";
    const succPaid =
      createdSettlement.paidAmount ||
      createdSettlement.paid_amount ||
      createdSettlement.netAmount ||
      effectivePaidAmount;
    const succRem =
      createdSettlement.remainingAmount ||
      createdSettlement.remaining_amount ||
      remainingBalance;
    const succDueDate =
      createdSettlement.expectedDueDate ||
      createdSettlement.expected_due_date ||
      expectedDueDate;
      ...
      <div className="max-w-2xl mx-auto py-10 animate-fadeIn">
        <div className={`p-8 text-center ${cardBg}`}>
          ...
          <p className={`text-xs mt-1.5 ${subText}`}>
            ...
            . Hari Om Finance will verify your UTR reference.
          </p>

          <div
            className={`my-6 p-5 rounded-[16px] text-left space-y-3 text-xs ${isDark ? "bg-white/[0.02]" : "bg-[#FAFAFA]"}`}
          >
            <div className="flex justify-between">
              <span className={subText}>Settlement ID:</span>
              <span className={`font-mono font-bold ${accentText}`}>
                {createdSettlement.settlement_number ||
                  createdSettlement.settlementNumber ||
                  createdSettlement.settlementId ||
                  createdSettlement.id ||
                  createdSettlement.referenceNumber}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-extrabold ${headingText}`}>
                ₹
                {Number(
                  createdSettlement.totalAmount ||
                    createdSettlement.total_amount ||
                ...
                {succDueDate && (
                  <div className="flex justify-between">
                    <span className={subText}>
                      Expected Balance Payment Due Date:
                    </span>
                    <span
                      className={`font-bold ${isDark ? "text-slate-200" : "text-[#111827]"}`}
                    >
                      {new Date(succDueDate).toLocaleDateString("en-IN", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-mono font-bold ${accentText}`}>
                {createdSettlement.reference_number ||
                  createdSettlement.referenceNumber}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span
                className={`font-bold ${isDark ? "text-slate-200" : "text-[#111827]"}`}
              >
                {createdSettlement.purchaseIds?.length || selectedIds.length}{" "}
                Purchases
              </span>
            </div>

            <div
              className={`flex justify-between items-center pt-2.5 border-t ${isDark ? "border-white/10" : "border-[#E5E7EB]"}`}
            >
              <span className={subText}>Current Status:</span>
              <span
                className={`text-[10px] font-extrabold px-3 py-1 rounded-lg ${
                  isPartialSuccess
                    ? "bg-[#FEF3C7] text-[#B45309]"
              ...
              >
                {isPartialSuccess
                  ? `Partial (₹${Number(succRem).toLocaleString("en-IN")} Pending Due)`
    ...
    <div className="space-y-8 animate-fadeIn pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1
            className={`text-2xl md:text-3xl font-extrabold tracking-tight mt-1.5 ${headingText}`}
          >
            Submit Remittance Batch
          </h1>
          <p className={`text-xs md:text-sm mt-1 ${subText}`}>
            Select outstanding course purchases, choose full or partial
            remittance mode, enter bank UTR, and set due date for remaining
            balance.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div
            className={`px-3.5 py-1.5 rounded-lg border text-xs font-bold flex items-center gap-2 ${
              selectedIds.length > 0
                ? isDark
                  ? "bg-[#3D5EF6]/20 border-[#3D5EF6]/40 text-blue-300"
                  : "bg-[#EEF1FE] border-[#3D5EF6]/30 text-[#3D5EF6]"
                : isDark
                  ? "bg-white/5 border-white/10 text-slate-400"
                  : "bg-[#F3F4F6] border-[#E5E7EB] text-[#6B7280]"
            }`}
          >
            <span className="w-5 h-5 rounded-full bg-[#3D5EF6] text-white flex items-center justify-center text-[10px] font-black">
              1
            </span>
            ...
            className={`px-3.5 py-1.5 rounded-lg border text-xs font-bold flex items-center gap-2 ${
              paymentMode === "partial"
                ? isDark
                  ...
                  : "bg-[#FEF3C7] border-amber-300 text-[#B45309]"
                : isDark
                  ? "bg-emerald-500/15 border-emerald-500/30 text-emerald-300"
                  : "bg-[#DCFCE7] border-emerald-300 text-[#16A34A]"
            }`}
          >
            <span
              className={`w-5 h-5 rounded-full text-white flex items-center justify-center text-[10px] font-black ${paymentMode === "partial" ? "bg-amber-600" : "bg-emerald-600"}`}
            >
              2
            </span>
      ...
      {error && (
        <div
          className={`p-4 rounded-[16px] flex items-center gap-3 text-xs font-bold ${
            isDark
      ...
      >
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-6 space-y-4">
            ...
            <div className={`p-6 space-y-5 ${cardBg}`}>
              <div
                className={`border-b pb-4 ${isDark ? "border-white/10" : "border-[#E5E7EB]"}`}
              >
                <span
                  className={`text-[10px] font-black uppercase tracking-wider ${accentText}`}
                >
                ...
                <p className={`text-xs mt-1 ${subText}`}>
                  Total payable by Partner to Hari Om for {selectedIds.length}{" "}
                  selected course purchases.
                </p>

                {/* Partial Payment Calculation Summary in Left Box */}
                {paymentMode === "partial" && (
                  <div
                    className={`mt-4 pt-3 border-t border-dashed space-y-2 text-xs ${isDark ? "border-white/10" : "border-[#E5E7EB]"}`}
                  >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span
                    className={`text-xs font-bold uppercase tracking-wider ${labelText}`}
                  >
                    Covered Items ({selectedPurchases.length} of{" "}
                    {purchases.length})
                  </span>
                  {purchases.length > 0 && (
                    <button
                ...
                {selectedPurchases.length === 0 ? (
                  <div
                    className={`p-4 text-center border border-dashed rounded-[16px] text-xs space-y-2 ${
                      isDark
                        ? "border-white/10 text-slate-400 bg-white/[0.01]"
                        : "border-[#E5E7EB] text-[#6B7280] bg-[#FAFAFA]"
                    }`}
                  >
                    <AlertCircle className="w-5 h-5 mx-auto text-amber-500 opacity-80" />
                    <p className={`font-bold ${headingText}`}>
                      No purchases selected yet
                    </p>
                    <p className={`text-[11px] ${subText}`}>
                      Check items from the list on the right to include them in
                      this remittance batch.
                    </p>
                    {purchases.length > 0 && (
                      <button
                        type="button"
                    ...
                    {selectedPurchases.map((p) => (
                      ...
                      >
                        <div className="min-w-0 flex-1 pr-2">
                          <p className={`font-bold truncate ${headingText}`}>
                            {p.seafarerName}
                          </p>
                          <p className={`text-[10px] truncate ${subText}`}>
                            {p.courseName}
                          </p>
                        </div>
                        <span
                          className={`font-extrabold font-mono shrink-0 ${accentText}`}
                        >
            ...
            <div className={`p-5 rounded-[16px] border-0 space-y-3 ${cardBg}`}>
              <div className="flex items-center gap-2">
                <div
                  className={`w-7 h-7 rounded-lg flex items-center justify-center ${isDark ? "bg-[#3D5EF6]/15 text-[#3D5EF6]" : "bg-[#EEF1FE] text-[#3D5EF6]"}`}
                >
                ...
                <div>
                  ...
                  <p className={`text-[10px] ${subText}`}>
                    Hari Om Remittance Process
                  </p>
                </div>
              </div>
              <div
                className={`text-[11px] space-y-2 leading-relaxed ${subText}`}
              >
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#3D5EF6] mt-1.5 shrink-0" />
                  <p>
                    Remittances are audited and verified within{" "}
                    <strong>2–4 business hours</strong> of UTR confirmation.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#16A34A] mt-1.5 shrink-0" />
                  <p>
                    Candidate course enrollments are confirmed immediately upon
                    verification.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  ...
                  <p>
                    For partial remittances, remaining balances must be settled
                    on or before the selected due date.
                  </p>
                </div>
              </div>
              <div
                className={`pt-2 border-t text-[11px] flex items-center justify-between ${isDark ? "border-white/10" : "border-[#E5E7EB]"}`}
              >
                <span className={subText}>Need assistance?</span>
                <Link
                  href="/partner/support"
                  className="font-bold text-[#3D5EF6] hover:underline"
                >
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className={`p-6 space-y-4 ${cardBg}`}>
            <div
              className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b ${
            ...
            >
              <div>
                <div className="flex items-center gap-2">
                  <h2
                    className={`text-base font-extrabold flex items-center gap-2 ${headingText}`}
                  >
                    <Receipt
                      className={`w-5 h-5 ${isDark ? "text-blue-400" : "text-[#3D5EF6]"}`}
                    />{" "}
                    1. Select Outstanding Purchases Covered
                  </h2>
                  <span
                    className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-lg ${
                      selectedIds.length > 0
                        ? isDark
                          ? "bg-[#3D5EF6]/20 text-blue-300"
                          : "bg-[#EEF1FE] text-[#3D5EF6]"
                        : isDark
                ...
                <p className={`text-xs mt-0.5 ${subText}`}>
                  Choose course purchases covered in your current bank
                  remittance batch
                </p>
              </div>
            ...
            {loading ? (
              <div className="p-8 text-center text-slate-400 animate-pulse">
                Loading eligible purchases...
              </div>
            ) : filteredPurchases.length === 0 ? (
              <div
                className={`p-8 text-center border border-dashed rounded-[16px] text-xs ${
                  isDark
              ...
              >
                ...
                <p className={`font-bold text-sm ${headingText}`}>
                  No matching outstanding purchases.
                </p>
                <p className="mt-1">
                  All purchases are either settled or no pending record matches
                  search.
                </p>
              </div>
            ) : (
              <div className="space-y-3 max-h-[28rem] overflow-y-auto pr-1 custom-scrollbar">
                {filteredPurchases.map((p, idx) => {
                  ...
                  const candidateIndos =
                    p.indosNumber || p.indosNum || "24IN9999";
                        ...
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <p
                                className={`font-extrabold text-xs ${headingText}`}
                              >
                                {p.seafarerName}
                              </p>
                              <span
                                className={`font-mono text-[10px] font-extrabold px-2.5 py-0.5 rounded-lg ${
                                  isDark
                            ...
                            <p
                              className={`text-[11px] mt-1 font-medium ${subText}`}
                            >
                              {p.courseName} • Enrolled{" "}
          ...
          <div className={`p-6 space-y-4 ${cardBg}`}>
            <div
              className={`pb-3 border-b ${isDark ? "border-white/10" : "border-[#E5E7EB]"}`}
            >
              <div className="flex items-center justify-between">
                <h2
                  className={`text-base font-extrabold flex items-center gap-2 ${headingText}`}
                >
                  <CreditCard
                    className={`w-5 h-5 ${isDark ? "text-blue-400" : "text-[#3D5EF6]"}`}
                  />{" "}
                  2. Bank Payment & Transaction Statement
                </h2>
                <span
                  className={`text-[10px] font-bold px-2.5 py-0.5 rounded-lg ${
                    isDark
                      ? "bg-[#3D5EF6]/15 text-blue-300"
                      : "bg-[#EEF1FE] text-[#3D5EF6]"
                  }`}
                >
              ...
              <p className={`text-xs mt-0.5 ${subText}`}>
                Enter your bank remittance reference, UTR number, and upload
                bank statement / transfer receipt proof for audit.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <p className={`text-[10px] mt-1 ${subText}`}>
                  Standard banking limits: UPI / IMPS (12 digits), NEFT (16
                  characters), RTGS (22 characters). Max 22 alphanumeric
                  characters.
                </p>
              </div>
              ...
              <div className="md:col-span-2">
                <label
                  className={`block text-xs mb-1.5 ${labelText} flex items-center justify-between`}
                >
                  <span>
                ...
                {bankStatementFile ? (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p
                          className={`text-xs font-bold truncate ${headingText}`}
                        >
                          {bankStatementFile.name}
                        </p>
                        <p className={`text-[10px] ${subText}`}>
                          {(bankStatementFile.size / 1024).toFixed(1)} KB •
                          Attached for finance audit
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <label
                    className={`border-2 border-dashed rounded-[16px] p-4 flex flex-col items-center justify-center text-center cursor-pointer transition-all ${
                      isDark
                  ...
                  >
                    ...
                    <p className={`text-[10px] mt-0.5 ${subText}`}>
                      Accepted: PDF, JPG, PNG (Bank slip, e-statement, or net
                      banking UTR screenshot)
                    </p>
                  </label>
                )}
                {statementError && (
              ...
              <div>
                ...
                >
                  ...
                  <option value="Cash / Direct Branch Deposit">
                    Direct Branch Cash Deposit
                  </option>
                </select>
              </div>
          ...
          <div className={`p-6 space-y-4 ${cardBg}`}>
            ...
            >
              ...
              >
                <PieChart
                  className={`w-5 h-5 ${isDark ? "text-blue-400" : "text-[#3D5EF6]"}`}
                />{" "}
                3. Remittance Payment Mode (Full vs Partial Split)
              </h2>
              <p className={`text-xs mt-0.5 ${subText}`}>
                Choose whether you are transferring the full batch amount (100%)
                or making a partial payment now and scheduling the remaining
                balance.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <CheckCircle2
                      className={`w-5 h-5 ${paymentMode === "full" ? "text-[#16A34A]" : "text-slate-400"}`}
                    />
                    <span className={`text-xs font-black ${headingText}`}>
                      Full Remittance (100%)
                    </span>
                  </div>
                ...
                <p className={`text-[11px] mt-2 leading-relaxed ${subText}`}>
                  Pay 100% total batch amount (₹
                  {totalAmount.toLocaleString("en-IN")}) in one bank transfer.
                  All courses are immediately submitted for audit.
                </p>
              </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Clock
                      className={`w-5 h-5 ${paymentMode === "partial" ? "text-[#B45309]" : "text-slate-400"}`}
                    />
                    <span className={`text-xs font-black ${headingText}`}>
                      Partial Remittance Split
                    </span>
                  </div>
                ...
                <p className={`text-[11px] mt-2 leading-relaxed ${subText}`}>
                  Pay part of the balance now via UTR, and schedule the
                  remaining balance to be remitted on or before a selected due
                  date.
                </p>
              </div>
            </div>
            ...
            {paymentMode === "partial" && (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label
                      className={`block text-xs font-bold mb-1.5 ${labelText}`}
                    >
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      Enter partial amount transferred in current UTR
                      transaction.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div
                      className={`w-full px-4 py-2.5 rounded-xl text-sm font-mono font-extrabold flex items-center justify-between border ${
                        isDark
                          ? "bg-black/30 border-slate-800 text-amber-300"
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      Remaining balance (₹{totalAmount.toLocaleString("en-IN")}{" "}
                      - ₹{effectivePaidAmount.toLocaleString("en-IN")})
                    </p>
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                          ...
                          >
                            ...
                            {Math.floor(
                              (effectivePaidAmount || totalAmount) /
                                Math.max(1, selectedPurchases.length),
                            ).toLocaleString("en-IN")}{" "}
                            each)
                          </button>
                        )}
                    ...
                    <div className="space-y-2.5 max-h-64 overflow-y-auto custom-scrollbar pr-0.5">
                      {itemizedAllocations.map((a, i) => {
                        ...
                        const currentPaid =
                          manualAllocations[a.id] !== undefined
                            ? manualAllocations[a.id]
                            : a.paidNow;
                        return (
                          <div
                            key={a.id}
                          ...
                          >
                            <div className="md:col-span-5 flex items-center gap-3 min-w-0">
                              ...
                              <div className="min-w-0 pr-1">
                                <p
                                  className={`font-bold text-xs truncate ${headingText}`}
                                >
                                  {a.seafarerName}
                                </p>
                                <p
                                  className={`text-[10px] truncate font-medium ${subText}`}
                                  title={a.courseName}
                                >
                                  {a.courseName}
                                </p>
                            ...
                            {allocationMode === "manual" ? (
                              ...
                              <div className="md:col-span-7 text-right shrink-0 font-mono">
                                ...
                                <p className="text-[10px] text-slate-500 font-semibold mt-0.5">
                                  {a.remainingDue > 0
                                    ? `Remaining Due: ₹${a.remainingDue.toLocaleString("en-IN")}`
                                    : "Fully Settled"}
                                </p>
                              </div>
                            )}
                  ...
                  <div className="md:col-span-2">
                    <label
                      className={`block text-xs mb-1.5 ${labelText} flex items-center gap-1.5`}
                    >
                      <Calendar className="w-3.5 h-3.5 text-[#3D5EF6]" />{" "}
                      Expected Due Date for Remaining Balance (₹
                      {remainingBalance.toLocaleString("en-IN")}) *
                    </label>
                    <input
                      type="date"
                    ...
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
                      Specify the date by which your agency will pay the
            ...
            <div
              className={`flex items-center justify-end gap-3 pt-4 border-t ${isDark ? "border-white/10" : "border-[#E5E7EB]"}`}
            >
              <Link
                href="/partner/settlements"
