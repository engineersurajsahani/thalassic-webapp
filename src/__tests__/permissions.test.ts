...
describe("Frontend Permission & Route Access Tests", () => {
  it("should allow MASTER full dashboard access", () => {
    ...
    expect(canAccessDashboard("MASTER", "/partner/dashboard")).toBe(true);
  });
  ...
  it("should restrict SEAFARER from accessing master or admin routes", () => {
    ...
    expect(canAccessDashboard("SEAFARER", "/company-admin/dashboard")).toBe(
      false,
    );
    expect(canAccessDashboard("SEAFARER", "/agent-admin/dashboard")).toBe(
      false,
    );
    expect(canAccessDashboard("SEAFARER", "/partner/dashboard")).toBe(false);
  ...
  it("should check role hierarchy correctly", () => {
    ...
    expect(hasPermission("MASTER", "PARTNER_ADMIN")).toBe(true);
    expect(hasPermission("MASTER", "PARTNER")).toBe(true);
